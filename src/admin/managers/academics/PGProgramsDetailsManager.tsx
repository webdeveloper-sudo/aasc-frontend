...
const PGProgramsDetailsManager: React.FC = () => {
  ...
  const [data, setData] = useState<PGProgram[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState("");

  const [isEditing, setIsEditing] = useState(false);
  const [isNew, setIsNew] = useState(false);
  const [editItem, setEditItem] = useState<PGProgram | null>(null);

  const collectionName = "academics/pgprogrammsdetails";
  ...
  const handleEdit = (item: PGProgram) => {
    setEditItem({ ...item });
    setIsNew(false);
    setIsEditing(true);
  };
  ...
  // FORM FIELD UPDATER
  // ---------------------------
  const updateField = (field: keyof PGProgram, value: any) => {
    if (!editItem) return;
    ...
    return (
      <div className="p-6">
        ...
        <div className="form-container border border-gray-300 rounded-lg p-6 bg-white max-w-4xl shadow-sm">
          <Heading
            title={isNew ? "Add New PG Programme" : "Edit PG Programme"}
            size="lg"
            align="left"
          />
    ...
    <div className="collection-manager p-6 min-h-screen">
      <div className=" mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
          <div>
            <Heading title="PG Programmes Management" size="lg" align="left" />
            <p className="text-sm text-gray-500 mt-1">
              Manage postgraduate courses and details
            </p>
          </div>
